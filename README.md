...
1. Siempre me intrigó que la primera opción para los modelos de regresión fuese minimizar los **errores cuadráticos** y no los **absolutos**. ¿Por qué la preferencia? ¿No suena más lógico usar los errores absolutos?
...
5. Adelantemos el final: ello es porque elegir *mínimos cuadrados ordinarios* (MCO) implica tomar la geometría de los **espacios euclideos**, mientras que el 
método de *mínimas desviaciones absolutas* (MAD) se asocia a una geometría más "rara" (**la "del taxista"**) ¿Qué es cada cosa?
...
8. Con dicha métrica, puntos equidistantes a otro forman círculos concéntricos y su distancia se calcula como la suma cuadrática de la diferencia de sus componentes (Pitágoras!) 
...
11. Tomemos nota para más adelante: minimizar la raíz de una función positiva es lo mismo que minimizar directamente la función (porque la raíz se vuelve monótona creciente). Por lo que, en el caso euclidiano, el problema planteado equivale a minimizar las diferencias cuadráticas
...
12. Notemos que los resultados de la minimización difieren con cada métrica (tanto en el punto encontrado como en su distancia). Con la euclidiana el punto más cercano (Pe) está sobre el **círculo tangente**, mientras que con la manhattan (Pm) está sobre el **rombo tangente** 
...
13. Por eso Pe se llama **"proyección ortogonal"** del vector Y sobre la recta. Vale remarcar que la perpendicularidad demanda una idea de "ángulo", que se define a partir de dos estructuras que mencionamos: el producto interior y la norma
...
16. En este punto remarco un hecho crucial cuya demostración escapa a este hilo: una desventaja de la distancia manhattan es que no es posible encuadrarla en una estructura geométrica semejante. Aunque es posible definir una norma, no hay un producto interior compatible


17. Por lo que no tenemos ángulos ni proyecciones ortogonales, fundamentales a la hora de definir (entre otros) el concepto de correlación. Peor aún, en espacios con esta métrica, los puntos de mínima distancia pueden no ser únicos
...
22. Lo bueno de este ejemplo es que podemos representarlo en 2D: en las abcisas ubicaremos la primera observación y en las ordenadas la segunda. Entonces y es el vector (2,5), y beta * x genera una recta desde el origen que pasa por x=(1,2).

<img src="images/MCO-step1.png" width="50%">

23. La recta es el espacio que representa los puntos "barridos" por el vector x con cada valor de beta. Y lo que queremos al estimar es, justamente, un beta tal que multiplizado por x = (1,2) esté *lo más cerca posible* de y = (2,5). ¡Minimizar la distancia entre el punto y la recta!
...
26. En cambio el MAD usa la distancia mahattan. La estimación y el beta se hallan trazando el rombo tangente en lugar del círculo. El error ya no es ortogonal a la recta.

<img src="images/MCO-step5.png" width="50%">

27. En suma: ambos métodos implican geometrías diferentes. Aunque el MCO parece más "raro", se empareja en realidad con nuestra noción usual de distancia (el viejo Pitágoras). Y este espacio permite definir nociones clave como la de proyección ortogonal, ausentes con la manhattan


28. Con más observaciones y regresores se hace imposible hacer una representación visual sencilla. Sin embargo, la interpretación geométrica sigue siendo válida. Por eso el operador para estimar con el MCO se llama "matriz de proyección" 
...
29. Esta interpretación se puede extender de espacios de vectores de datos (el MCO más elemental) a variables aleatorias. Con ese fin hay que pensar en espacios de funciones (las variables aleatorias son funciones medibles), pero *fantásticamente* la geometría es la misma.

30. Los espacios euclidianos generalizados se llaman espacios de Hilbert (con norma L2) y la distancia Manhattan vive en los espacios de Banach (con norma L1). Los primeros mantienen una ventaja en simplicidad porque tienen producto interior, ángulos y proyecciones ortogonales 


31. Bonus track: la geometría de los espacios de Hilbert unifica campos de estudio tan diversos como la i) probabilidad, la estadística y la econometría (la proyección ortogonal es la base para definir la esperanza condicional, por ejemplo)
